...
const nativePlatform = window.Capacitor && typeof window.Capacitor.getPlatform === 'function'
  ...
  : '';
const API_BASE_STORAGE_KEY = 'FIELDTRACK_API_BASE_OVERRIDE';
...
const BACKEND_BASE = String(getStoredApiBase() || window.FIELDTRACK_API_BASE || defaultBackendBase).replace(/\/$/, '');
window.__FIELDTRACK_API_BASE = BACKEND_BASE;
window.FIELDTRACK_API_BASE = BACKEND_BASE;
...
throw new Error(`Cannot connect to ${BACKEND_BASE}. Check the server address below and that the backend is running.`);
}
...
if (apiBaseInput) {
apiBaseInput.value = BACKEND_BASE;
}
if (apiBaseForm) {
apiBaseForm.addEventListener('submit', function (event) {
event.preventDefault();
const value = apiBaseInput ? apiBaseInput.value : '';
const clean = setStoredApiBase(value);
if (apiBaseStatus) {
apiBaseStatus.textContent = clean
? `Saved. Using ${clean}. Reloading...`
: 'Cleared. Using the built-in default. Reloading...';
}
setTimeout(function () {
...
getSession,
clearSession,
getApiBase: function () { return BACKEND_BASE; },
